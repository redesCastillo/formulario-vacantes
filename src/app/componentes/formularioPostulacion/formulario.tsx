        ...
        if (!res) {
            ...
            try {
                const enviarCorreo = await fetch("/api/send", {
                    ...
                    body: JSON.stringify({datos: data, pdf, titulo: res}),
                })
                        ...
                        <div className="form-group">
                            ...
                            <div className="file-upload-wrapper">
                                <div className="file-upload">
                                    <CldUploadWidget options={{clientAllowedFormats: ["pdf"]}} uploadPreset="app-formulario" onSuccess={(res) => {
                                            if (typeof res.info === "object" && "secure_url" in res.info) {
                                                setPdf((res.info as { secure_url: string }).secure_url);
                                            }
                                        }} >
                                            {({open}) => {
                                                return <button className="file-upload-label" onClick={() => open()}>Sube tu curriculum</button>
                                            }}
                                    </CldUploadWidget>
                                </div>
